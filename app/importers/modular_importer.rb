...
class ModularImporter
  ...
  def import
    attrs = {
      ...
      depositor_id: '1',       # pass a Hyrax user_key here and that Hyrax user will own all objects created during this import
    ...
    csv_import_detail = {
      csv_import_detail:Zizia::CsvImportDetail.new(attributes: attrs)
    }
    file = File.open(@csv_file)
    parser = Zizia::CsvParser.new(file: file)
    record_importer = Zizia::HyraxRecordImporter.new(attributes: csv_import_detail)
